...
- §11.2 / §11.2.4 的 `limit-up-board` 数据需求矩阵 → v0.8 起以 [`docs/limit-up-board.md`](limit-up-board.md) 为准；具体地，`top_list` / `top_inst` / `cyq_perf` 由 optional 升级为 required（Phase B 的 LHB + 筹码因子是 R2 决策核心输入），yaml `permissions.tushare_apis.required` 与 `data.py` 已对齐
- §18.4 的 `ChannelContext` → §0.2 已统一更名为 `PluginContext`，并由 `plugins_api/base.py` 持有；`plugins_api/channel.py` 仅定义 `ChannelPlugin` Protocol（其 `push(ctx: PluginContext, ...)` 复用 base 中的 dataclass）
...
def step1_collect_round1(ctx, trade_date, params):
    ...
    # 2b) v0.4 — 流通市值 < N 亿 + 股价 < M 元（默认 100/15，user 可通过
    #     `deeptrade limit-up-board settings` 持久化修改；null 字段 → 过滤）
